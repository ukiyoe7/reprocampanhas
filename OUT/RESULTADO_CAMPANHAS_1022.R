...
CP_1419_1022 %>% summarize(v=sum(BONUS))

## OBS


OBS_1419 <- paste0("LOOK ",CP_1419_1022  %>% 
                     distinct(CLICODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y"))


## JOIN CPF

LIST_1419_1022 <- inner_join(CP_1419_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_1419)
...
PAG_1419_1022 <- LIST_1419_1022 %>%  group_by(CPF) %>% 
...
CP_305_1022 %>% summarize(v=sum(BONUS))


## OBS

OBS_305 <- paste0("GARUVA ",CP_305_1022 %>% 
                    distinct(CLICODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y"))


## JOIN CPF

LIST_305_1022 <- inner_join(CP_305_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_305)
...
LIST_305_1022 %>% summarize(v=sum(BONUS))


### pagamentos

PAG_305_1022 <- LIST_305_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_305) 
...
PAG_305_1022 %>% summarize(v=sum(BONUS)) 
...
CP_G121_1022 %>% summarize(v=sum(VRVENDA))

CP_G121_1022 %>% summarize(v=sum(BONUS))


## OBS

OBS_G121 <- paste0("RINALDI ","G",CP_G121_1022 %>% 
                     distinct(GCLCODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_G121_1022 <- inner_join(CP_G121_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_G121)
...
PAG_G121_1022 <- LIST_G121_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>%
  mutate(OBS=OBS_G121) 
...
PAG_G121_1022 %>% summarize(v=sum(BONUS))
...
CP_206_1022 %>% summarize(v=sum(VRVENDA))

CP_206_1022 %>% summarize(v=sum(BONUS))


OBS_206 <-  paste0("OTICA RH ",CP_206_1022 %>% distinct(CLICODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 

## JOIN CPF

LIST_206_1022 <- inner_join(CP_206_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_206)
...
PAG_206_1022 <- LIST_206_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_206)
...
CP_1225_1022 %>% summarize(v=sum(VRVENDA))

CP_1225_1022 %>% summarize(v=sum(BONUS))

OBS_1225 <- paste0("VINI ",CP_1225_1022 %>% 
                     distinct(CLICODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_1225_1022  <- inner_join(CP_1225_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_1225)
...
PAG_1225_1022 <- LIST_1225_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_1225) 
...
PAG_1225_1022 %>% summarize(v=sum(BONUS))
...
CP_G224_1022 %>% summarize(v=sum(VRVENDA))

CP_G224_1022 %>% summarize(v=sum(BONUS))

## OBSERV

OBS_G224 <- paste0("ITACU ","G",CP_G224_1022 %>% 
                     distinct(GCLCODIGO), " ",format(floor_date(Sys.Date()-months(1),"month"),"%m/%y")) 


## JOIN CPF

LIST_G224_1022  <- inner_join(CP_G224_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_G224)
...
PAG_G224_1022 <- LIST_G224_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_G224) 
...
PAG_G224_1022 %>% summarize(v=sum(BONUS))
...
CP_G77_1022 %>% summarize(v=sum(VRVENDA))

CP_G77_1022 %>% summarize(v=sum(BONUS))

## OBSERV

OBS_G77 <- paste0("DANJU ","G",CP_G77_1022 %>% 
                    distinct(GCLCODIGO), " ",format(floor_date(Sys.Date()-months(1),"month"),"%m/%y")) 


## JOIN CPF

LIST_G77_1022  <- inner_join(CP_G77_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_G77)
...
PAG_G77_1022 <- LIST_G77_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_G77) 
...
PAG_G77_1022 %>% summarize(v=sum(BONUS))


## 905 VIVA =====================================================================================================

CP_905_1022 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_905_1022 %>% summarize(v=sum(VRVENDA))

CP_905_1022 %>% summarize(v=sum(BONUS))

## OBSERV

OBS_905 <- paste0("VIVA ",CP_905_1022 %>% 
                    distinct(CLICODIGO), " ",format(floor_date(Sys.Date()-months(1),"month"),"%m/%y")) 


## JOIN CPF

LIST_905_1022  <- inner_join(CP_905_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_905)
...
PAG_905_1022 <- LIST_905_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_905) 
...
PAG_905_1022 %>% summarize(v=sum(BONUS))

## 1360 VIVA =============================================================================================

CP_1360_1022 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_1360_1022 %>% summarize(v=sum(VRVENDA))

CP_1360_1022 %>% summarize(v=sum(BONUS))

## OBSERV

OBS_1360 <- paste0("VIVA ",CP_1360_1022 %>% 
                     distinct(CLICODIGO), " ",format(floor_date(Sys.Date()-months(1),"month"),"%m/%y")) 


## JOIN CPF

LIST_1360_1022  <- inner_join(CP_1360_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_1360)
...
PAG_1360_1022 <- LIST_1360_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_1360) 
...
PAG_1360_1022 %>% summarize(v=sum(BONUS))


## 360 UNIVERSAL =============================================================================================


CP_360_1022 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_360_1022 %>% summarize(v=sum(VRVENDA))

CP_360_1022 %>% summarize(v=sum(BONUS))

OBS_360 <- paste0("UNIVERSAL ",CP_360_1022 %>% 
                    distinct(CLICODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m/%y")) 


## JOIN CPF

LIST_360_1022  <- inner_join(CP_360_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_360)
...
PAG_360_1022 <- LIST_360_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_360) 
...
PAG_360_1022 %>% summarize(v=sum(BONUS))
## ====================================================================================================

# BARBARA K G37

CP_G37_1022 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_G37_1022 %>% summarize(v=sum(VRVENDA))

CP_G37_1022 %>% summarize(v=sum(BONUS))

OBS_G37_1022 <- paste0("BARBARA K G",CP_G37_1022 %>% 
                         distinct(GCLCODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_G37_1022  <- inner_join(CP_G37_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_G37_1022)
...
PAG_G37_1022 <- LIST_G37_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_G37_1022 ) 
...
PAG_G37_1022 %>% summarize(v=sum(BONUS))


left_join(PAG_G37_1022 %>%  mutate(CPF=as.character(CPF)),CARTOES_1022,by="CPF") %>% View()

## =============================================================================================================         

# SQL G46 BBZ

CP_G46_1022 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
OBS_G46_1022 <- paste0("BBZ G",CP_G46_1022 %>% 
                         distinct(GCLCODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_G46_1022  <- inner_join(CP_G46_1022,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_G46_1022)
...
PAG_G46_1022 <- LIST_G46_1022 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_G46_1022 ) 
...
PAG_G46_1022 %>% summarize(v=sum(BONUS))


left_join(PAG_G46_1022 %>%  mutate(CPF=as.character(CPF)),CARTOES_1022,by="CPF") %>% View()
...
range_write(PAG_ALL_1022,ss="1xwI5n5SP8AG_ALHI7YnIMvdpVyDBXQq--2tdWrCXUgQ",range = "A1",sheet="PAGAMENTOS",reformat = FALSE)  
...
LIST_ALL_1022 %>% summarize(v=sum(BONUS))

range_write(LIST_ALL_1022,ss="1xwI5n5SP8AG_ALHI7YnIMvdpVyDBXQq--2tdWrCXUgQ",range = "A:P",sheet="PEDIDOS",reformat = FALSE)  

## ==============================================================================================================
## CREDITO CARTOES

CREDITO_CARTOES_1022 <- left_join(PAG_ALL_1022 %>%
                                    mutate(CPF=as.character(CPF)),CARTOES_1022,by="CPF") 


View(CREDITO_CARTOES_1022)



CREDITO_CARTOES_1022_2 <- CREDITO_CARTOES_1022 %>% 
  filter(!is.na(`Número do Série`))

View(CREDITO_CARTOES_1022_2)

CREDITO_CARTOES_1022_3 <- CREDITO_CARTOES_1022_2 %>% 
  .[,c(7,1,2,3)] %>% 
  rename_at(1:4, ~ c("Número de Série","CPF","Valor da Carga","Observacao"))


View(CREDITO_CARTOES_1022_3)  

CREDITO_CARTOES_1022_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_1022_3 %>% .[duplicated(.$CPF),]


write.csv2(CREDITO_CARTOES_1022_3,
           file = "C:\\Users\\Repro\\Documents\\R\\ADM\\CAMPANHAS_REPRO\\OUT\\CREDITO_CARTOES_1022.csv",
           row.names=FALSE,quote = FALSE)


left_join(CREDITO_CARTOES_1022_2,ALELO_1022 %>% rename(NSERIE=`Número de Série`),by="NSERIE") %>%.[,c(-4,-5,-6,-7,-8,-9)] %>% View()
...
EMISSAO_CARTOES_1022 <- CREDITO_CARTOES_1022 %>% 
  filter(is.na(NSERIE)) %>% 
...
View(EMISSAO_CARTOES_1022)


range_write("1aF4Z-yN83Wj_14LwNVe1ubBb-kWAAIE8Znvkaj7O-XI",data =EMISSAO_CARTOES_1022,
