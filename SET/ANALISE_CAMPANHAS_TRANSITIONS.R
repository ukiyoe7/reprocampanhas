...
CLIENTES_TGEN8 <- read_sheet(ss="1-GoZYN4QGXT346g2mT0jTyAw_6TqKHQAa_0VS1_Fhms",sheet = "CLIENTES")
...
CP_TRANSITIONS_0607_21_sql <- glue_sql("
...
CLI AS(SELECT C.CLICODIGO,CLINOMEFANT,GCLCODIGO,SETOR FROM CLIEN C
       ...
       C.CLICODIGO=A.CLICODIGO WHERE C.CLICODIGO IN ({CLIENTES_TGEN8$CLICODIGO*}) ),
...
CP_TRANSITIONS_0607_21<-  dbGetQuery(con2,CP_TRANSITIONS_0607_21_sql) 

View(CP_TRANSITIONS_0607_21)

## ===================================================================

CP_TRANSITIONS_0809_21_sql <- glue_sql("
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_TRANSITIONS_0809_21<-  dbGetQuery(con2,CP_TRANSITIONS_0809_21_sql) 


View(CP_TRANSITIONS_0809_21)


## ============================================================


CP_TRANSITIONS_0607_22_sql <- glue_sql("
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_TRANSITIONS_0607_22<-  dbGetQuery(con2,CP_TRANSITIONS_0607_22_sql) 

View(CP_TRANSITIONS_0607_22)


CP_TRANSITIONS_0809_22_sql <- glue_sql("
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_TRANSITIONS_0809_22<-  dbGetQuery(con2,CP_TRANSITIONS_0809_22_sql) 

View(CP_TRANSITIONS_0809_22)

## QTD

QTD_0607_21 <- CP_TRANSITIONS_0607_21 %>% 
                group_by(CLICODIGO) %>% 
                 summarize(QTD_0607_21=sum(QTD)) 

QTD_0809_21 <- CP_TRANSITIONS_0809_21 %>% 
                group_by(CLICODIGO) %>% 
                  summarize(QTD_0809_21=sum(QTD)) 

QTD_0607_22 <- CP_TRANSITIONS_0607_22 %>% 
                group_by(CLICODIGO) %>% 
                 summarize(QTD_0607_22=sum(QTD)) 
 
QTD_0809_22 <- CP_TRANSITIONS_0809_22 %>% 
                group_by(CLICODIGO) %>% 
                 summarize(QTD_0809_22=sum(QTD)) 

CP_QTD <- left_join(CLIENTES_TGEN8,QTD_0607_21,by="CLICODIGO") %>% 
            left_join(.,QTD_0809_21,by="CLICODIGO")  %>% 
              left_join(.,QTD_0607_22,by="CLICODIGO")  %>% 
               left_join(.,QTD_0809_22,by="CLICODIGO") %>%  
                arrange(desc(QTD_0809_22))

range_write(CP_QTD,ss="1IIB6DduSKjoPAhaFks3QVxKrgS6a74Bsa0Acn6ZasE0",range = "A:G",sheet = "QTD",reformat = FALSE)



## BONUS

BONUS_0607_21 <- CP_TRANSITIONS_0607_21 %>% 
  group_by(CLICODIGO) %>% 
  summarize(BONUS_0607_21=sum(BONUS)) 

BONUS_0809_21 <- CP_TRANSITIONS_0809_21 %>% 
  group_by(CLICODIGO) %>% 
  summarize(BONUS_0809_21=sum(BONUS)) 

BONUS_0607_22 <- CP_TRANSITIONS_0607_22 %>% 
  group_by(CLICODIGO) %>% 
  summarize(BONUS_0607_22=sum(BONUS)) 

BONUS_0809_22 <- CP_TRANSITIONS_0809_22 %>% 
  group_by(CLICODIGO) %>% 
  summarize(BONUS_0809_22=sum(BONUS)) 

CP_BONUS <- left_join(CLIENTES_TGEN8,BONUS_0607_21,by="CLICODIGO") %>% 
  left_join(.,BONUS_0809_21,by="CLICODIGO")  %>% 
  left_join(.,BONUS_0607_22,by="CLICODIGO")  %>% 
  left_join(.,BONUS_0809_22,by="CLICODIGO") %>%  
  arrange(desc(BONUS_0809_22))


range_write(CP_BONUS,ss="1IIB6DduSKjoPAhaFks3QVxKrgS6a74Bsa0Acn6ZasE0",range = "A:G",sheet = "BONUS",reformat = FALSE)
