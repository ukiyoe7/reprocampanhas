...
CP_788_0922 %>% summarize(v=sum(VRVENDA))

CP_788_0922 %>% summarize(v=sum(BONUS))

OBS_788 <- paste0("ASTRO ",CP_788_0922 %>% 
                    distinct(CLICODIGO)," ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_788_0922  <- inner_join(CP_788_0922,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_788)
...
PAG_788_0922 <- LIST_788_0922 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_788) 
...
PAG_788_0922 %>% summarize(v=sum(BONUS))
...
range_write(LIST_ALL_0922,ss="1Ri4NCSv4zO1iSxj4AqzgQtxthf1qbnz4MSzz3OpVsRg",range = "A:P",sheet="PEDIDOS",reformat = FALSE)  


## CREDITO CARTOES

CREDITO_CARTOES_1_0922 <- left_join(PAG_ALL_0922 %>%
