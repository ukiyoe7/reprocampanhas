...
CP_2148_0822 %>% summarize(v=sum(VRVENDA))

CP_2148_0822 %>% summarize(v=sum(BONUS))

## OBSERV

OBS_2148 <- paste0("VIVA ",CP_2148_0822 %>% 
                    distinct(CLICODIGO), " ",format(floor_date(Sys.Date()-months(1),"month"),"%m/%y")) 


## JOIN CPF

LIST_2148_0822  <- inner_join(CP_2148_0822,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_2148)
...
PAG_2148_0822 <- LIST_2148_0822 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_2148) 
...
PAG_2148_0822 %>% summarize(v=sum(BONUS))
