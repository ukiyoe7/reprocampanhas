...
## 360 UNIVERSAL ATE 31.12.2023 =============================================================================================


CP_VARILUX_360_0124<- dbGetQuery(con2,"
...
View(CP_VARILUX_360_0124)

CP_VARILUX_360_0124 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_360_0124 %>% summarize(v=sum(BONUS))

OBS_VARILUX_360_0124 <- paste0("UNIVERSAL 360 ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_VARILUX_360_0124  <- inner_join(CP_VARILUX_360_0124,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_360_0124) %>% mutate(CPF=as.character(CPF)) 

View(LIST_VARILUX_360_0124)


### PAY

PAG_VARILUX_360_0124 <- LIST_VARILUX_360_0124 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_360_0124) %>% mutate(CPF=as.character(CPF)) 

View(PAG_VARILUX_360_0124)


PAG_VARILUX_360_0124 %>% summarize(v=sum(BONUS))

write.csv2(LIST_VARILUX_360_0124 ,file = "C:\\Users\\REPRO SANDRO\\Documents\\R\\REPRO CAMPANHAS\\2024\\JAN\\LIST_VARILUX_360_0124.csv" ,row.names = FALSE,na="")

write.csv2(PAG_VARILUX_360_0124,file = "C:\\Users\\REPRO SANDRO\\Documents\\R\\REPRO CAMPANHAS\\2024\\JAN\\PAG_VARILUX_360_0124.csv",,row.names = FALSE,na="")
