...
## CAMPANHA VARILUX ECONOMICA 291 RETROATIVA 21.11.23 - 30.11.23 =====================================


CP_291_VLX_ECONO_1123 <- 
...
CP_291_VLX_ECONO_1123 %>% summarize(v=sum(VRVENDA))

CP_291_VLX_ECONO_1123%>% summarize(v=sum(BONUS))


OBS_291_VLX_ECONO_1123 <-  paste0("OTICA LUZ 291 VARILUX ECONOMICA RETROATIVO 1123") 

## JOIN CPF

LIST_291_VLX_ECONO_1123 <- inner_join(CP_291_VLX_ECONO_1123,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF=CPF.y) %>% .[,-11] %>% mutate(OBS=OBS_291_VLX_ECONO_1123)


View(LIST_291_VLX_ECONO_1123)

### pagamentos

PAG_291_VLX_ECONO_1123 <- LIST_291_VLX_ECONO_1123 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_291_VLX_ECONO_1123)


View(PAG_291_VLX_ECONO_1123)

write.csv2(LIST_291_VLX_ECONO_1123 %>% mutate(CPF=as.character(CPF)),file="C:\\Users\\REPRO SANDRO\\Documents\\R\\REPRO CAMPANHAS\\2024\\JAN\\LIST_291_VLX_ECONO_1123.csv",row.names = FALSE,na="")

write.csv2(PAG_291_VLX_ECONO_1123 %>% mutate(CPF=as.character(CPF)),file="C:\\Users\\REPRO SANDRO\\Documents\\R\\REPRO CAMPANHAS\\2024\\JAN\\PAG_291_VLX_ECONO_1123.csv",row.names = FALSE,na="")
