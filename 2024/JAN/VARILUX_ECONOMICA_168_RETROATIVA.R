...
## CAMPANHA VARILUX ECONOMICA 168 RETROATIVA 01.10.23 - 31.12.23 =====================================


CP_168_VLX_ECONO_1023_1223 <- 
  dbGetQuery(con2,"WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_168_VLX_ECONO_1023_1223 %>% summarize(v=sum(VRVENDA))

CP_168_VLX_ECONO_1023_1223 %>% summarize(v=sum(BONUS))


OBS_168_VLX_ECONO_1023_1223 <-  paste0("OTICA ORIENTE 168 VARILUX ECONOMICA RETROATIVO 1023 1223") 

## JOIN CPF

LIST_168_VLX_ECONO_1023_1223 <- inner_join(CP_168_VLX_ECONO_1023_1223,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF=CPF.y) %>% .[,-11] %>% mutate(OBS=OBS_168_VLX_ECONO_1023_1223)


View(LIST_168_VLX_ECONO_1023_1223)

### pagamentos

PAG_168_VLX_ECONO_1023_1223 <- LIST_168_VLX_ECONO_1023_1223 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_168_VLX_ECONO_1023_1223)


View(PAG_168_VLX_ECONO_1023_1223)

write.csv2(LIST_168_VLX_ECONO_1023_1223 %>% mutate(CPF=as.character(CPF)),file="C:\\Users\\REPRO SANDRO\\Documents\\R\\REPRO CAMPANHAS\\2024\\JAN\\LIST_168_VLX_ECONO_1023_1223.csv",row.names = FALSE,na="")

write.csv2(PAG_168_VLX_ECONO_1023_1223 %>% mutate(CPF=as.character(CPF)),file="C:\\Users\\REPRO SANDRO\\Documents\\R\\REPRO CAMPANHAS\\2024\\JAN\\PAG_168_VLX_ECONO_1023_1223.csv",row.names = FALSE,na="")
