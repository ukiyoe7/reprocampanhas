...
CP_INSIGNE_0524 <-dbGetQuery(con2,"
...
LIST_INSIGNE_0524_ALL <- left_join(RESULT_INSIGNE_0524,BASE_CPF,by="CLICODIGO") %>%
  rename(CPF=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  mutate(CPF3=ifelse(is.na(CPF2),CPF,CPF2)) 

## PAY ==================================================================

PAG_INSIGNE_0524 <-  LIST_INSIGNE_0524_ALL %>% filter(nchar(CPF3)==11) %>% group_by(CPF3,OBS) %>% 
...
PAG_INSIGNE_0524 %>% summarize(v=sum(BONUS))


View(PAG_INSIGNE_0524)

## LISTAGEM FINAL  =============================================================================================================         

LIST_INSIGNE_0524 <-  rbind( 
  LIST_INSIGNE_0524_ALL
) %>%  .[,c(-12,-16)] %>% rename(CPF=CPF3)

View(LIST_INSIGNE_0524)
...
CREDITO_CARTOES_INSIGNE_0524 <- left_join(PAG_INSIGNE_0524 %>%
...
CREDITO_CARTOES_INSIGNE_0524_2 <- CREDITO_CARTOES_INSIGNE_0524 %>% 
...
CREDITO_CARTOES_INSIGNE_0524_3 <- CREDITO_CARTOES_INSIGNE_0524_2  %>% 
  .[,c(6,1,3,2)] %>% 
...
file_path <- paste0("C:\\Users\\REPRO SANDRO\\OneDrive - Luxottica Group S.p.A (1)\\CAMPANHAS ALELO\\", current_year, "\\", current_month, "\\CREDITO_CARTOES_INSIGNE_0524.csv")


# Write CSV with updated file path
write.csv2(CREDITO_CARTOES_INSIGNE_0524_3,
           file = file_path,
           row.names = FALSE,
           quote = FALSE)
...
EMISSAO_CARTOES_INSIGNE_0524_1 <-
  inner_join(
    
    CREDITO_CARTOES_INSIGNE_0524 %>% 
      filter(is.na(NSERIE)) %>% filter(PGTO_MINIMO=='S') %>%  
    ...
    CREDITO_CARTOES_INSIGNE_0524 %>% 
      ...
      left_join(.,PARTICIPANTES_CAMPANHA %>% 
                  mutate(CPF=as.character(CPF)),by="CPF") %>% 
      group_by(CPF) %>% 
      summarize(`Carimbo de data/hora` = max(`Carimbo de data/hora`), 
