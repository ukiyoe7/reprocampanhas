...
clientes_mpr_0324 <- dbGetQuery(con2,"SELECT CP.CLICODIGO,CLINOMEFANT,GCLCODIGO FROM CLIPROMO CP
                                                LEFT JOIN CLIEN C ON C.CLICODIGO=CP.CLICODIGO
                                                WHERE ID_PROMO=44
                                            ")

View(clientes_mpr_0324)


## SQL ============================================================

SQL_MREPRO_0324 <- dbGetQuery(con2,"WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
CP_1464_MREPRO_0324 %>% summarize(v=sum(VRVENDA))

CP_1464_MREPRO_0324%>% summarize(v=sum(BONUS))


OBS_1464_MREPRO_0324 <-  paste0("OTICA ESPECIALISTA 1464 MARCA REPRO ",format(floor_date(Sys.Date(),"month") - months(1),"%m%y")) 

## JOIN CPF

LIST_1464_MREPRO_0324 <- inner_join(CP_1464_MREPRO_0324,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF=CPF.y) %>% .[,-11] %>% mutate(OBS=OBS_1464_MREPRO_0324)


### pagamentos

PAG_1464_MREPRO_0324 <- LIST_1464_MREPRO_0324 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_1464_MREPRO_0324)


View(PAG_1464_MREPRO_0324)
...
CP_360_MREPRO_0324 %>% summarize(v=sum(VRVENDA))

CP_360_MREPRO_0324%>% summarize(v=sum(BONUS))


OBS_360_MREPRO_0324 <-  paste0("OTICA UNIVERSAL 360 MARCA REPRO ",format(floor_date(Sys.Date(),"month") - months(1),"%m%y")) 

## JOIN CPF

LIST_360_MREPRO_0324 <- inner_join(CP_360_MREPRO_0324,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF=CPF.y) %>% .[,-11] %>% mutate(OBS=OBS_360_MREPRO_0324)


### pagamentos

PAG_360_MREPRO_0324 <- LIST_360_MREPRO_0324 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_360_MREPRO_0324)


View(PAG_360_MREPRO_0324)
...
CP_35_MREPRO_0324 %>% summarize(v=sum(VRVENDA))

CP_35_MREPRO_0324%>% summarize(v=sum(BONUS))


OBS_35_MREPRO_0324 <-  paste0("OTICA ODOLNI 35 MARCA REPRO ",format(floor_date(Sys.Date(),"month") - months(1),"%m%y")) 

## JOIN CPF

LIST_35_MREPRO_0324 <- inner_join(CP_35_MREPRO_0324,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF=CPF.y) %>% .[,-11] %>% mutate(OBS=OBS_35_MREPRO_0324)


### pagamentos

PAG_35_MREPRO_0324 <- LIST_35_MREPRO_0324 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_35_MREPRO_0324)


View(PAG_35_MREPRO_0324)
...
View(PAG_MREPRO_0324)

PAG_MREPRO_0324 %>% summarize(v=sum(BONUS))
...
LIST_MREPRO_0324 <-  rbind(
  LIST_1464_MREPRO_0324,
  LIST_360_MREPRO_0324,
  LIST_35_MREPRO_0324
) 

View(LIST_MREPRO_0324)
...
CREDITO_CARTOES_MREPRO_0324 <- left_join(PAG_MREPRO_0324 %>%
...
View(CREDITO_CARTOES_MREPRO_0324)

## EXCLUI SEM CARTAO

CREDITO_CARTOES_MREPRO_0324_2 <- CREDITO_CARTOES_MREPRO_0324 %>% 
  filter(!is.na(NSERIE)) %>% filter(PGTO_MINIMO=='S')

View(CREDITO_CARTOES_MREPRO_0324_2)


## CRIA BASE DE PAGAMENTO

CREDITO_CARTOES_MREPRO_0324_3 <- CREDITO_CARTOES_MREPRO_0324_2 %>% 
  .[,c(6,1,2,3)] %>% 
  rename_at(1:4, ~ c("Número de Série","CPF","Valor da Carga","Observacao"))


View(CREDITO_CARTOES_MREPRO_0324_3)  

CREDITO_CARTOES_MREPRO_0324_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_MREPRO_0324_3 %>% .[duplicated(.$CPF),]
...
file_path <- paste0("C:\\Users\\REPRO SANDRO\\OneDrive - Luxottica Group S.p.A (1)\\CAMPANHAS ALELO\\", current_year, "\\", current_month, "\\CREDITO_CARTOES_MREPRO_0324.csv")


# Write CSV with updated file path
write.csv2(CREDITO_CARTOES_MREPRO_0324_3,
           file = file_path,
           row.names = FALSE,
           quote = FALSE)
