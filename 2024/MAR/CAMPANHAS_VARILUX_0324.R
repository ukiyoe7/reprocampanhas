...
CP_VARILUX_G121_0324 <- dbGetQuery(con2,"
...
View(CP_VARILUX_G121_0324)

CP_VARILUX_G121_0324 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_G121_0324 %>% summarize(v=sum(BONUS))

## OBS

OBS_VARILUX_G121_0324 <- paste0("RINALDI G121 ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_VARILUX_G121_0324 <- inner_join(CP_VARILUX_G121_0324,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>%
  .[,c(-2,-13)] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_G121_0324)


### pagamentos

PAG_VARILUX_G121_0324 <- LIST_VARILUX_G121_0324 %>% group_by(CPF) %>% 
...
VARILUX_G224_0324 <- dbGetQuery(con2,"
...
View(VARILUX_G224_0324)

VARILUX_G224_0324 %>% summarize(v=sum(VRVENDA))

VARILUX_G224_0324 %>% summarize(v=sum(BONUS))


OBS_VARILUX_G224_0324 <-  paste0("OTICA ITAÇU G224 TODOS VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## ASSOCIATE CPF

LIST_VARILUX_G224_0324 <- 
  VARILUX_G224_0324 %>% mutate(CPF='00524459924') %>% 
  mutate(OBS=OBS_VARILUX_G224_0324)



### pagamentos

PAG_VARILUX_G224_0324 <- LIST_VARILUX_G224_0324 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_G224_0324) 
...
PAG_VARILUX_0324 <-  
  rbind(
    PAG_VARILUX_G121_0324,
    PAG_VARILUX_G224_0324) %>% 
  mutate(PGTO_MINIMO=if_else(BONUS>=100,"S","N")) %>% 
  mutate(TIPO="VARILUX")


PAG_VARILUX_0324 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_0324 <-  rbind(
  LIST_VARILUX_G121_0324,
  LIST_VARILUX_G224_0324) 


LIST_VARILUX_0324 %>% summarize(v=sum(BONUS))
...
CREDITO_CARTOES_VARILUX_0324 <- left_join(PAG_VARILUX_0324 %>%
...
View(CREDITO_CARTOES_VARILUX_0324)

## EXCLUI SEM CARTAO

CREDITO_CARTOES_VARILUX_0324_2 <- CREDITO_CARTOES_VARILUX_0324 %>% 
  filter(!is.na(NSERIE)) %>% filter(PGTO_MINIMO=='S')

View(CREDITO_CARTOES_VARILUX_0324_2)


## CRIA BASE DE PAGAMENTO

CREDITO_CARTOES_VARILUX_0324_3 <- CREDITO_CARTOES_VARILUX_0324_2 %>% 
  .[,c(6,1,2,3)] %>% 
  rename_at(1:4, ~ c("Número de Série","CPF","Valor da Carga","Observacao"))


View(CREDITO_CARTOES_VARILUX_0324_3)  

CREDITO_CARTOES_VARILUX_0324_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_VARILUX_0324_3 %>% .[duplicated(.$CPF),]
...
file_path <- paste0("C:\\Users\\REPRO SANDRO\\OneDrive - Luxottica Group S.p.A (1)\\CAMPANHAS ALELO\\", current_year, "\\", current_month, "\\CREDITO_CARTOES_VARILUX_0324.csv")


# Write CSV with updated file path
write.csv2(CREDITO_CARTOES_VARILUX_0324_3,
           file = file_path,
           row.names = FALSE,
           quote = FALSE)
