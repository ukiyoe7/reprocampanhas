...
CP_INSIGNE_0523 <-dbGetQuery(con2,"
...
CP_INSIGNE_0523 %>% summarize(v=sum(VRVENDA))

CP_INSIGNE_0523 %>% summarize(v=sum(BONUS))

CP_INSIGNE_0523 %>% filter(CLICODIGO==151) %>% summarize(v=sum(BONUS))



##REMOVE DUPLICATES

CP_INSIGNE_0523_2 <- CP_INSIGNE_0523 %>% filter(CLICODIGO!=213)

CP_INSIGNE_0523_3 <- CP_INSIGNE_0523_2 %>% filter(CLICODIGO!=151)

## INTERSECTION

LIST_INSIGNE_0523 <- left_join(CP_INSIGNE_0523_3,BASE_CPF,by="CLICODIGO") %>%
...
PAG_INSIGNE_0523 <-  LIST_INSIGNE_0523 %>% filter(nchar(CPF3)==11) %>% group_by(CPF3) %>% 
...
CP_INSIGNE_0523_151 <- CP_INSIGNE_0523 %>% filter(CLICODIGO==151)

View(CP_INSIGNE_0523_151)

LIST_INSIGNE_0523_151 <- left_join(CP_INSIGNE_0523_151,BASE_CPF,by="CLICODIGO") %>%
  rename(CPF=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  mutate(OBS=OBS_SIG) %>% 
  mutate(CPF3=ifelse(CPF2=='NULL',CPF,CPF2)) %>%  
  mutate(CPF3=ifelse(nchar(gsub("[^[:alnum:]]", "", PEDAUTORIZOU)) < 6,'00773625941',CPF2))

View(LIST_INSIGNE_0523_151)

PAG_INSIGNE_0523_151 <-  LIST_INSIGNE_0523_151 %>% group_by(CPF3) %>% 
  summarize(BONUS=sum(BONUS)) %>% 
  mutate(OBS=OBS_SIG) %>% rename(CPF=CPF3)

View(PAG_INSIGNE_0523_151)

## 213  ========

CP_INSIGNE_0523_213 <- CP_INSIGNE_0523 %>% filter(CLICODIGO==213)

## INTERSECT

LIST_INSIGNE_0523_213 <- left_join(CP_INSIGNE_0523_213,BASE_CPF,by="CLICODIGO") %>%
  rename(CPF=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  mutate(OBS=OBS_SIG) %>% mutate(CPF3=ifelse(CPF2=='NULL',CPF,CPF2)) 

## PAY

PAG_INSIGNE_0523_213 <-  LIST_INSIGNE_0523_213 %>% group_by(CPF3) %>% 
  summarize(BONUS=sum(BONUS)/(LIST_INSIGNE_0523_213 %>% distinct(CPF3) %>% lengths())) %>% 
  mutate(OBS=OBS_SIG) %>% rename(CPF=CPF3)

## PAGAMENTOS ======================================================================================================= 


PAG_INSIGNE_0523_ALL <-  rbind( 
  PAG_INSIGNE_0523,
...
PAG_INSIGNE_0523_ALL %>% summarize(v=sum(BONUS))


range_write(PAG_INSIGNE_0523_ALL,ss="1a-u9ZH9RLQnQGrCUHzDRpao0EEuzhfSlBfgHAn27ry8",range = "A16",
            col_names = FALSE,sheet="RESUMO",reformat = FALSE)  


## LISTAGEM FINAL  =============================================================================================================         

LIST_INSIGNE_0523_ALL <-  rbind( 
  LIST_INSIGNE_0523,
  LIST_INSIGNE_0523_213,
  LIST_INSIGNE_0523_151
) 

View(LIST_INSIGNE_0523_ALL)
...
View(CREDITO_CARTOES_INSIGNE_0523)
...
CREDITO_CARTOES_INSIGNE_0523_2 %>% .[duplicated(.$CPF),] 
...
View(CREDITO_CARTOES_INSIGNE_0523_3)  

CREDITO_CARTOES_INSIGNE_0523_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_INSIGNE_0523_3 %>% .[duplicated(.$CPF),]
...
EMISSAO_CARTOES_INSIGNE_0523_1 <- CREDITO_CARTOES_INSIGNE_0523 %>% 
  filter(is.na(NSERIE)) %>%
  left_join(.,PARTICIPANTES_CAMPANHA %>% 
              mutate(CPF=as.character(CPF)),by="CPF") %>% 
  .[,c(1:3,11:14)] 

View(EMISSAO_CARTOES_INSIGNE_0523_1)

range_write("1a-u9ZH9RLQnQGrCUHzDRpao0EEuzhfSlBfgHAn27ry8",data =EMISSAO_CARTOES_INSIGNE_0523_1,
...
CP_INSIGNE_0523_DIGITADOS <-dbGetQuery(con2,"
WITH CLI AS (SELECT DISTINCT C.CLICODIGO,CLINOMEFANT,GCLCODIGO,SETOR
  FROM CLIEN C
  INNER JOIN (SELECT CLICODIGO FROM CLIPROMO WHERE ID_PROMO=18)CLP ON C.CLICODIGO=CLP.CLICODIGO
  LEFT JOIN (SELECT CLICODIGO,D.ZOCODIGO,ZODESCRICAO SETOR FROM ENDCLI D
...
View(CP_INSIGNE_0523_DIGITADOS)
...
view(insigne_pagos_0523)




ALELO_0523_INSIGNE <- ALELO_0523 %>% rename(OBS=6) %>% 
  filter(str_detect(OBS,"INSIGNE")) %>% 
  rename(NSERIE=4)

View(ALELO_0523_INSIGNE)



PAG_ALELO_0523 <- left_join(ALELO_0523_INSIGNE,CARTOES_0523 %>% 
                              rename(NSERIE=5) %>% mutate(as.character(CPF)),by="NSERIE")

View(PAG_ALELO_0523) 


left_join(insigne_pagos_0523 ,PAG_ALELO_0523,by="CPF") %>% View()
