...
CP_VARILUX_G121_0523 <- dbGetQuery(con2,"
...
View(CP_VARILUX_G121_0523)

CP_VARILUX_G121_0523 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_G121_0523 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_G121_0523_G <- inner_join(CP_VARILUX_G121_0523 %>% 
...
View(LIST_VARILUX_G121_0523_G)


range_write(LIST_VARILUX_G121_0523_G,ss="1a-u9ZH9RLQnQGrCUHzDRpao0EEuzhfSlBfgHAn27ry8",range = "A1",
...
LIST_VARILUX_G121_0523 <- inner_join(CP_VARILUX_G121_0523,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>%
  .[,c(-2,-13)] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_G121)

View(LIST_VARILUX_G121_0523)


### pagamentos

PAG_VARILUX_G121_0523 <- LIST_VARILUX_G121_0523 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>%
  mutate(OBS=OBS_VARILUX_G121) 

View(PAG_VARILUX_G121_0523)

# Calc Bonus

PAG_VARILUX_G121_0523 %>% summarize(v=sum(BONUS))
...
CP_206_0523 %>% summarize(v=sum(VRVENDA))

CP_206_0523 %>% summarize(v=sum(BONUS))


OBS_206 <-  paste0("OTICA RH ",CP_206_0523 %>% distinct(CLICODIGO)," ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 

## JOIN CPF

LIST_VARILUX_206_0523 <- inner_join(CP_206_0523,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_206)
...
View(LIST_VARILUX_206_0523)


### pagamentos

PAG_VARILUX_206_0523 <- LIST_VARILUX_206_0523 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_206)


View(PAG_VARILUX_206_0523)
...
CP_VARILUX_1225_0523 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_1225_0523)

CP_VARILUX_1225_0523 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_1225_0523 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_1225_0523  <- inner_join(CP_VARILUX_1225_0523,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_1225)

View(LIST_VARILUX_1225_0523)


### PAY

PAG_VARILUX_1225_0523 <- LIST_VARILUX_1225_0523 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_1225) 

View(PAG_VARILUX_1225_0523)


PAG_VARILUX_1225_0523 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_360_0523 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_360_0523)

CP_VARILUX_360_0523 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_360_0523 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_360_0523  <- inner_join(CP_VARILUX_360_0523,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_360)

View(LIST_VARILUX_360_0523)


### PAY

PAG_VARILUX_360_0523 <- LIST_VARILUX_360_0523 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_360) 

View(PAG_VARILUX_360_0523)


PAG_VARILUX_360_0523 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_4576_0523 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_4576_0523)

CP_VARILUX_4576_0523 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_4576_0523 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_4576_0523  <- inner_join(CP_VARILUX_4576_0523,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_4576)

View(LIST_VARILUX_4576_0523)


### PAY

PAG_VARILUX_4576_0523 <- LIST_VARILUX_4576_0523 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_4576) 

View(PAG_VARILUX_4576_0523)


PAG_VARILUX_4576_0523 %>% summarize(v=sum(BONUS))
...
PAG_VARILUX_0523 <-  
  rbind(
    PAG_VARILUX_G121_0523, 
    PAG_VARILUX_206_0523, 
...
LIST_VARILUX_0523 <-  rbind(
  LIST_VARILUX_G121_0523,
  LIST_VARILUX_206_0523,
  LIST_VARILUX_1225_0523, 
  LIST_VARILUX_360_0523,
  LIST_VARILUX_4576_0523
) 



View(LIST_VARILUX_0523)

LIST_VARILUX_0523 %>% summarize(v=sum(BONUS))

range_write(LIST_VARILUX_0523,ss="1a-u9ZH9RLQnQGrCUHzDRpao0EEuzhfSlBfgHAn27ry8",
...
View(CREDITO_CARTOES_VARILUX_0523_3)  

CREDITO_CARTOES_VARILUX_0523_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_VARILUX_0523_3 %>% .[duplicated(.$CPF),]
...
EMISSAO_CARTOES_0523 <- CREDITO_CARTOES_VARILUX_0523 %>% 
  filter(is.na(NSERIE)) %>% 
  left_join(.,PARTICIPANTES_CAMPANHA %>% 
              mutate(CPF=as.character(CPF)),by="CPF") %>% 
  .[,c(1:3,11:14)] 

View(EMISSAO_CARTOES_0523)


range_write("1_HkZd1o8SGeNbC8Hw8gB3LPsqiqRhsqTdvYznwFFUYw",data =EMISSAO_CARTOES_0523,
