...
CP_G45_0823 %>% summarize(v=sum(VRVENDA))

CP_G45_0823 %>% summarize(v=sum(BONUS))
...
LIST_G45_0823 <- inner_join(CP_G45_0823,BASE_CPF,by="CLICODIGO") %>% 
...
PAG_G45_0823 <- LIST_G45_0823 %>% group_by(CPF) %>% 
...
View(PAG_OUTROS_0823)

PAG_OUTROS_0823 %>% summarize(v=sum(BONUS))

range_write(PAG_OUTROS_0823,ss="1bgKkW2aReox24kSi0FvsUQD-b0rOk9RkCXrb66hFhpM",range = "A63",
...
LIST_OUTROS_0823 <-  rbind(
  LIST_G45_0823
) 

View(LIST_OUTROS_0823)

LIST_OUTROS_0823%>% summarize(v=sum(BONUS))

range_write(LIST_OUTROS_0823,ss="1bgKkW2aReox24kSi0FvsUQD-b0rOk9RkCXrb66hFhpM",
            range = "A:P",sheet="OUTROS",reformat = FALSE)  
