...
CP_VARILUX_G121_0823 <- dbGetQuery(con2,"
...
View(CP_VARILUX_G121_0823)

CP_VARILUX_G121_0823 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_G121_0823 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_G121_0823_G <- inner_join(CP_VARILUX_G121_0823 %>% 
...
View(LIST_VARILUX_G121_0823_G)


range_write(LIST_VARILUX_G121_0823_G,ss="1Fy7pehis2ZCDdy42lF3gVuKmDZhRVSSVF40W8GkKbsI",range = "A1",
...
LIST_VARILUX_G121_0823 <- inner_join(CP_VARILUX_G121_0823,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>%
  .[,c(-2,-13)] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_G121)

View(LIST_VARILUX_G121_0823)


### pagamentos

PAG_VARILUX_G121_0823 <- LIST_VARILUX_G121_0823 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>%
  mutate(OBS=OBS_VARILUX_G121) 

View(PAG_VARILUX_G121_0823)

# Calc Bonus

PAG_VARILUX_G121_0823 %>% summarize(v=sum(BONUS))
...
CP_206_0823 %>% summarize(v=sum(VRVENDA))

CP_206_0823 %>% summarize(v=sum(BONUS))


OBS_206 <-  paste0("OTICA RH ",CP_206_0823 %>% distinct(CLICODIGO)," ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 

## JOIN CPF

LIST_VARILUX_206_0823 <- inner_join(CP_206_0823,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_206)
...
View(LIST_VARILUX_206_0823)


### pagamentos

PAG_VARILUX_206_0823 <- LIST_VARILUX_206_0823 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_206)


View(PAG_VARILUX_206_0823)
...
CP_VARILUX_1225_0823 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_1225_0823)

CP_VARILUX_1225_0823 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_1225_0823 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_1225_0823  <- inner_join(CP_VARILUX_1225_0823,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_1225)

View(LIST_VARILUX_1225_0823)


### PAY

PAG_VARILUX_1225_0823 <- LIST_VARILUX_1225_0823 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_1225) 

View(PAG_VARILUX_1225_0823)


PAG_VARILUX_1225_0823 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_360_0823 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_360_0823)

CP_VARILUX_360_0823 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_360_0823 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_360_0823  <- inner_join(CP_VARILUX_360_0823,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_360)

View(LIST_VARILUX_360_0823)


### PAY

PAG_VARILUX_360_0823 <- LIST_VARILUX_360_0823 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_360) 

View(PAG_VARILUX_360_0823)


PAG_VARILUX_360_0823 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_4576_0823 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_4576_0823)

CP_VARILUX_4576_0823 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_4576_0823 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_4576_0823  <- inner_join(CP_VARILUX_4576_0823,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_4576)

View(LIST_VARILUX_4576_0823)


### PAY

PAG_VARILUX_4576_0823 <- LIST_VARILUX_4576_0823 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)-75) %>% 
  mutate(OBS=OBS_VARILUX_4576) 

View(PAG_VARILUX_4576_0823)


PAG_VARILUX_4576_0823 %>% summarize(v=sum(BONUS))
...
PAG_VARILUX_0823 <-  
  rbind(
    PAG_VARILUX_G121_0823, 
    PAG_VARILUX_206_0823, 
    PAG_VARILUX_1225_0823,
    PAG_VARILUX_360_0823,
    PAG_VARILUX_4576_0823
  ) %>% mutate(PGTO_MINIMO=if_else(BONUS>=100,"S","N")) %>% 
  mutate(TIPO="VARILUX")


View(PAG_VARILUX_0823)

PAG_VARILUX_0823 %>% summarize(v=sum(BONUS))

range_write(PAG_VARILUX_0823,ss="1Fy7pehis2ZCDdy42lF3gVuKmDZhRVSSVF40W8GkKbsI",range = "A15",
...
LIST_VARILUX_0823 <-  rbind(
  LIST_VARILUX_G121_0823,
  LIST_VARILUX_206_0823,
  LIST_VARILUX_1225_0823, 
  LIST_VARILUX_360_0823,
  LIST_VARILUX_4576_0823
) 



View(LIST_VARILUX_0823)

LIST_VARILUX_0823 %>% summarize(v=sum(BONUS))

range_write(LIST_VARILUX_0823,ss="1Fy7pehis2ZCDdy42lF3gVuKmDZhRVSSVF40W8GkKbsI",
...
CREDITO_CARTOES_VARILUX_0823 <- left_join(PAG_VARILUX_0823 %>%
...
View(CREDITO_CARTOES_VARILUX_0823)

## EXCLUI SEM CARTAO

CREDITO_CARTOES_VARILUX_0823_2 <- CREDITO_CARTOES_VARILUX_0823 %>% 
  filter(!is.na(NSERIE)) %>% filter(PGTO_MINIMO=='S')

View(CREDITO_CARTOES_VARILUX_0823_2)


## CRIA BASE DE PAGAMENTO

CREDITO_CARTOES_VARILUX_0823_3 <- CREDITO_CARTOES_VARILUX_0823_2 %>% 
  .[,c(6,1,2,3)] %>% 
  rename_at(1:4, ~ c("Número de Série","CPF","Valor da Carga","Observacao"))


View(CREDITO_CARTOES_VARILUX_0823_3)  

CREDITO_CARTOES_VARILUX_0823_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_VARILUX_0823_3 %>% .[duplicated(.$CPF),]


write.csv2(CREDITO_CARTOES_VARILUX_0823_3,
           file = "C:\\Users\\Repro\\Documents\\R\\ADM\\CAMPANHAS_REPRO\\2023\\JUL\\CREDITO_CARTOES_VARILUX_0823.csv",
...
NAO_BONIFICADOS_VARILUX_0823 <-
  
  left_join(CREDITO_CARTOES_VARILUX_0823 %>% 
              filter(!is.na(NSERIE)) %>% 
...
range_write("1Fy7pehis2ZCDdy42lF3gVuKmDZhRVSSVF40W8GkKbsI",data =NAO_BONIFICADOS_VARILUX_0823,
...
VARILUX_EMISSAO_CARTOES_0823 <- CREDITO_CARTOES_VARILUX_0823 %>% 
  filter(is.na(NSERIE)) %>% 
  left_join(.,PARTICIPANTES_CAMPANHA %>% 
              mutate(CPF=as.character(CPF)),by="CPF") %>%  .[,c(1:3,11:14)] 

View(VARILUX_EMISSAO_CARTOES_0823)


range_write("1Fy7pehis2ZCDdy42lF3gVuKmDZhRVSSVF40W8GkKbsI",data =EMISSAO_CARTOES_0823,
