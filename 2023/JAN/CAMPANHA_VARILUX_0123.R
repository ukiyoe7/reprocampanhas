...
CP_G121_0123 %>% summarize(v=sum(VRVENDA))

CP_G121_0123 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_G121_0123 <- inner_join(CP_G121_0123,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_G121)

View(LIST_VARILUX_G121_0123)


### pagamentos

PAG_VARILUX_G121_0123 <- LIST_VARILUX_G121_0123 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>%
  mutate(OBS=OBS_G121) 

View(PAG_VARILUX_G121_0123)

# Calc Bonus

PAG_VARILUX_G121_0123 %>% summarize(v=sum(BONUS))
...
CP_206_0123 %>% summarize(v=sum(VRVENDA))

CP_206_0123 %>% summarize(v=sum(BONUS))


OBS_206 <-  paste0("OTICA RH ",CP_206_0123 %>% distinct(CLICODIGO)," ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 

## JOIN CPF

LIST_VARILUX_206_0123 <- inner_join(CP_206_0123,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_206)
...
PAG_VARILUX_206_0123 <- LIST_VARILUX_206_0123 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_206)


View(PAG_VARILUX_206_0123)
...
CP_VARILUX_1225_0123 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_1225_0123)

CP_VARILUX_1225_0123 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_1225_0123 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_1225_0123  <- inner_join(CP_VARILUX_1225_0123,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_1225)

View(LIST_VARILUX_1225_0123)


### PAY

PAG_VARILUX_1225_0123 <- LIST_VARILUX_1225_0123 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_1225) 

View(PAG_VARILUX_1225_0123)


PAG_VARILUX_1225_0123 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_360_0123 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_360_0123)

CP_VARILUX_360_0123 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_360_0123 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_360_0123  <- inner_join(CP_VARILUX_360_0123,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_360)

View(LIST_VARILUX_360_0123)


### PAY

PAG_VARILUX_360_0123 <- LIST_VARILUX_360_0123 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_360) 

View(PAG_VARILUX_360_0123)


PAG_VARILUX_360_0123 %>% summarize(v=sum(BONUS))


## CP8 BLUE EYE =============================================================================================


CP_VARILUX_4253_0123 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
PROD4 AS (SELECT PROCODIGO,PRODESCRICAO FROM PRODU WHERE MARCODIGO=57 AND (PRODESCRICAO LIKE '%VARILUX X DESIGN%')),

PROD5 AS (SELECT PROCODIGO,PRODESCRICAO FROM PRODU WHERE MARCODIGO=57 AND (PRODESCRICAO LIKE '%PHYSIO 360%')),
...
View(CP_VARILUX_4253_0123)

CP_VARILUX_4253_0123 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_4253_0123 %>% summarize(v=sum(BONUS))

OBS_VARILUX_4253 <- paste0("BLUEEYE 4253 ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 


## JOIN CPF

LIST_VARILUX_4253_0123  <- inner_join(CP_VARILUX_4253_0123,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_4253)

View(LIST_VARILUX_4253_0123)


### PAY

PAG_VARILUX_4253_0123 <- LIST_VARILUX_4253_0123 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_4253) 

View(PAG_VARILUX_4253_0123)


PAG_VARILUX_4253_0123 %>% summarize(v=sum(BONUS))
...
PAG_VARILUX_0123 <-  
  rbind(
    PAG_VARILUX_G121_0123, #OK
    PAG_VARILUX_206_0123, #OK
    PAG_VARILUX_1225_0123, #OK
    PAG_VARILUX_360_0123, #OK
    PAG_VARILUX_4253_0123 #OK
  )


View(PAG_VARILUX_0123)

PAG_VARILUX_0123 %>% summarize(v=sum(BONUS))

range_write(PAG_VARILUX_0123,ss="1TwpOhu9lr_Us8_hpFz-A7GzfIIzpL3XzIsKTfKHA_B4",range = "A10",
...
LIST_VARILUX_0123 <-  rbind(
  LIST_VARILUX_G121_0123, #OK
  LIST_VARILUX_206_0123, #OK 
  LIST_VARILUX_1225_0123, #OK
  LIST_VARILUX_360_0123, #OK
  LIST_VARILUX_4253_0123 #OK
) 

View(LIST_VARILUX_0123)

LIST_VARILUX_0123 %>% summarize(v=sum(BONUS))

range_write(LIST_VARILUX_0123,ss="1TwpOhu9lr_Us8_hpFz-A7GzfIIzpL3XzIsKTfKHA_B4",range = "A:P",sheet="VARILUX",reformat = FALSE)  

## ==============================================================================================================
## CREDITO CARTOES

CREDITO_CARTOES_0123 <- left_join(PAG_VARILUX_0123 %>%
                                    mutate(CPF=as.character(CPF)),CARTOES_0123,by="CPF") 


View(CREDITO_CARTOES_0123)



CREDITO_CARTOES_0123_2 <- CREDITO_CARTOES_0123 %>% 
  filter(!is.na(`NSERIE`))

View(CREDITO_CARTOES_0123_2)

CREDITO_CARTOES_0123_3 <- CREDITO_CARTOES_0123_2 %>% 
  .[,c(5,1,2,3)] %>% 
  rename_at(1:4, ~ c("Número de Série","CPF","Valor da Carga","Observacao"))


View(CREDITO_CARTOES_0123_3)  

CREDITO_CARTOES_0123_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_0123_3 %>% .[duplicated(.$CPF),]


write.csv2(CREDITO_CARTOES_0123_3,
           file = "C:\\Users\\Repro\\Documents\\R\\ADM\\CAMPANHAS_REPRO\\NOV\\CREDITO_CARTOES_VARILUX_0123.csv",
           row.names=FALSE,quote = FALSE)


left_join(CREDITO_CARTOES_0123_2,ALELO_0123 %>% rename(NSERIE=`Número de Série`),by="NSERIE") %>%.[,c(-4,-5,-6,-7,-8,-9)] %>% View()
...
EMISSAO_CARTOES_0123 <- CREDITO_CARTOES_0123 %>% 
  filter(is.na(NSERIE)) %>% 
...
View(EMISSAO_CARTOES_0123)


range_write("1nwmb96gjl_2cRuAGFDzzWFhzxKhaiInStQs6UXvbI6Q",data =EMISSAO_CARTOES_0123,
