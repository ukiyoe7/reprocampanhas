...
CP_TRANSITIONS_G163_sql <- glue_sql("
...
CLI AS(SELECT C.CLICODIGO,CLINOMEFANT,GCLCODIGO,SETOR FROM CLIEN C
       ...
       C.CLICODIGO=A.CLICODIGO WHERE C.CLICODIGO IN (SELECT CLICODIGO FROM CLIEN WHERE GCLCODIGO=163)),
...
CP_TRANSITIONS_G163<-  dbGetQuery(con2,CP_TRANSITIONS_G163_sql) 


View(CP_TRANSITIONS_G163)

CP_TRANSITIONS_G163 %>% summarize(v=sum(VRVENDA))

CP_TRANSITIONS_G163 %>% summarize(v=sum(BONUS))



OBS_CP_TRANSITIONS_G163 <- paste0("TRANSITIONS PONTOS ESSILOR G163 RETROATIVO OUT DEZ 22") 


## PEDIDOS

LIST_CP_TRANSITIONS_G163   <- CP_TRANSITIONS_G163 %>%  mutate(OBS=OBS_CP_TRANSITIONS_G163)

View(LIST_CP_TRANSITIONS_G163)


## RESULTADO GERAL

RESULT_TRANSITIONS_G163 <- CP_TRANSITIONS_G163 %>% 
                            group_by(CLICODIGO) %>% 
                             summarize(BONUS=sum(BONUS)) %>% 
                              mutate(OBS=OBS_CP_TRANSITIONS_G163)

View(RESULT_TRANSITIONS_G163)

range_write(RESULT_TRANSITIONS_G163,ss="19j_BvpfGOSAs-_yDxH9oadSRjlMrMDSj6zdOGDcytUU",range = "A1",sheet = "RESUMO")



#VERIFICA IDs REPETIDOS

CP_TRANSITIONS_G163 %>% .[duplicated(.$ID_PEDIDO),]
...
range_write(LIST_CP_TRANSITIONS_G163,ss="19j_BvpfGOSAs-_yDxH9oadSRjlMrMDSj6zdOGDcytUU",range = "A1",sheet = "PEDIDOS")
