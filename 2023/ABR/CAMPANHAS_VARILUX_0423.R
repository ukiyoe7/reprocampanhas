...
CP_VARILUX_G121_0423 <- dbGetQuery(con2,"
...
View(CP_VARILUX_G121_0423)

CP_VARILUX_G121_0423 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_G121_0423 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_G121_0423_G <- inner_join(CP_VARILUX_G121_0423 %>% 
...
View(LIST_VARILUX_G121_0423_G)


range_write(LIST_VARILUX_G121_0423_G,ss="1-WNd3PxF5xhxy1Z2xO0EnEP0ujyzIxYr0h6eO6DgU_g",range = "A1",
...
LIST_VARILUX_G121_0423 <- inner_join(CP_VARILUX_G121_0423,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>%
  .[,c(-2,-13)] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_G121)

View(LIST_VARILUX_G121_0423)


### pagamentos

PAG_VARILUX_G121_0423 <- LIST_VARILUX_G121_0423 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>%
  mutate(OBS=OBS_VARILUX_G121) 

View(PAG_VARILUX_G121_0423)

# Calc Bonus

PAG_VARILUX_G121_0423 %>% summarize(v=sum(BONUS))
...
CP_206_0423 %>% summarize(v=sum(VRVENDA))

CP_206_0423 %>% summarize(v=sum(BONUS))


OBS_206 <-  paste0("OTICA RH ",CP_206_0423 %>% distinct(CLICODIGO)," ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 

## JOIN CPF

LIST_VARILUX_206_0423 <- inner_join(CP_206_0423,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_206)
...
View(LIST_VARILUX_206_0423)


### pagamentos

PAG_VARILUX_206_0423 <- LIST_VARILUX_206_0423 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_206)


View(PAG_VARILUX_206_0423)
...
CP_VARILUX_1225_0423 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_1225_0423)

CP_VARILUX_1225_0423 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_1225_0423 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_1225_0423  <- inner_join(CP_VARILUX_1225_0423,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_1225)

View(LIST_VARILUX_1225_0423)


### PAY

PAG_VARILUX_1225_0423 <- LIST_VARILUX_1225_0423 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_1225) 

View(PAG_VARILUX_1225_0423)


PAG_VARILUX_1225_0423 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_360_0423 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_360_0423)

CP_VARILUX_360_0423 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_360_0423 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_360_0423  <- inner_join(CP_VARILUX_360_0423,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_360)

View(LIST_VARILUX_360_0423)


### PAY

PAG_VARILUX_360_0423 <- LIST_VARILUX_360_0423 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_360) 

View(PAG_VARILUX_360_0423)


PAG_VARILUX_360_0423 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_4576_0423 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_4576_0423)

CP_VARILUX_4576_0423 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_4576_0423 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_4576_0423  <- inner_join(CP_VARILUX_4576_0423,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_4576)

View(LIST_VARILUX_4576_0423)


### PAY

PAG_VARILUX_4576_0423 <- LIST_VARILUX_4576_0423 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_4576) 

View(PAG_VARILUX_4576_0423)


PAG_VARILUX_4576_0423 %>% summarize(v=sum(BONUS))
...
PAG_VARILUX_0423 <-  
  rbind(
    PAG_VARILUX_G121_0423, 
    PAG_VARILUX_206_0423, 
    PAG_VARILUX_1225_0423,
    PAG_VARILUX_360_0423,
    PAG_VARILUX_4576_0423
  )


View(PAG_VARILUX_0423)

PAG_VARILUX_0423 %>% summarize(v=sum(BONUS))

range_write(PAG_VARILUX_0423,ss="1-WNd3PxF5xhxy1Z2xO0EnEP0ujyzIxYr0h6eO6DgU_g",range = "A14",
...
LIST_VARILUX_0423 <-  rbind(
  LIST_VARILUX_G121_0423,
  LIST_VARILUX_206_0423,
  LIST_VARILUX_1225_0423, 
  LIST_VARILUX_360_0423,
  LIST_VARILUX_4576_0423
) 

 

View(LIST_VARILUX_0423)

LIST_VARILUX_0423 %>% summarize(v=sum(BONUS))

range_write(LIST_VARILUX_0423,ss="1-WNd3PxF5xhxy1Z2xO0EnEP0ujyzIxYr0h6eO6DgU_g",
...
CREDITO_CARTOES_VARILUX_0423 <- left_join(PAG_VARILUX_0423 %>%
...
View(CREDITO_CARTOES_VARILUX_0423)



CREDITO_CARTOES_VARILUX_0423_2 <- CREDITO_CARTOES_VARILUX_0423 %>% 
  filter(!is.na(NSERIE))

View(CREDITO_CARTOES_VARILUX_0423_2)

CREDITO_CARTOES_VARILUX_0423_3 <- CREDITO_CARTOES_VARILUX_0423_2 %>% 
  .[,c(4,1,2,3)] %>% 
  rename_at(1:4, ~ c("Número de Série","CPF","Valor da Carga","Observacao"))


View(CREDITO_CARTOES_VARILUX_0423_3)  

CREDITO_CARTOES_VARILUX_0423_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_VARILUX_0423_3 %>% .[duplicated(.$CPF),]


write.csv2(CREDITO_CARTOES_VARILUX_0423_3,
           file = "C:\\Users\\Repro\\Documents\\R\\ADM\\CAMPANHAS_REPRO\\2023\\ABR\\CREDITO_CARTOES_VARILUX_0423.csv",
           row.names=FALSE,quote = FALSE)


left_join(CREDITO_CARTOES_0423_2,ALELO_0423 %>% rename(NSERIE=`Número de Série`),by="NSERIE") %>%.[,c(-4,-5,-6,-7,-8,-9)] %>% View()
...
EMISSAO_CARTOES_0423 <- CREDITO_CARTOES_VARILUX_0423 %>% 
  filter(is.na(NSERIE)) %>% 
  left_join(.,PARTICIPANTES_CAMPANHA %>% 
              mutate(CPF=as.character(CPF)),by="CPF") %>% 
  .[,c(1:3,11:14)] 

View(EMISSAO_CARTOES_0423)


range_write("1_HkZd1o8SGeNbC8Hw8gB3LPsqiqRhsqTdvYznwFFUYw",data =EMISSAO_CARTOES_0423,
