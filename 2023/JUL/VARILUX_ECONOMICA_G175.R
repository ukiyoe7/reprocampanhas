## CAMPANHA TRANSITIONS ECONOMICA G175 RETROATIVO
...
query_vlx_G175_0623_retroativo <- dbGetQuery(con2,"
...
CLI AS(SELECT C.CLICODIGO,CLINOMEFANT,GCLCODIGO,SETOR FROM CLIEN C
       ...
       C.CLICODIGO=A.CLICODIGO WHERE GCLCODIGO IN (175)),
...
PROD6 AS (SELECT PROCODIGO,PRODESCRICAO,50 BONUS FROM PRODU WHERE  
                  ((PRODESCRICAO LIKE '%EYEZEN%' AND 
...
PROD10 AS (SELECT PROCODIGO,PRODESCRICAO,50 BONUS FROM PRODU WHERE  
             (PRODESCRICAO LIKE '%LIBERTY%' AND PRODESCRICAO LIKE '%AIR%')
             AND
                  (PRODESCRICAO LIKE '%TGEN8%' OR PRODESCRICAO LIKE '%TRANS%') AND
                 GR1CODIGO<>17 AND PROSITUACAO='A'
...
PROD16 AS (SELECT PROCODIGO,PRODESCRICAO,50 BONUS FROM PRODU WHERE  
             (PRODESCRICAO LIKE '%COMFORT%' AND PRODESCRICAO LIKE '%AIR%')
             AND
                  (PRODESCRICAO LIKE '%TGEN8%' OR PRODESCRICAO LIKE '%TRANS%') AND
                 GR1CODIGO<>17 AND PROSITUACAO='A'
...
PROD22 AS (SELECT PROCODIGO,PRODESCRICAO,50 BONUS FROM PRODU WHERE  
             (PRODESCRICAO LIKE '%VARILUX E%' AND PRODESCRICAO LIKE '%AIR%')
             AND
             (PRODESCRICAO LIKE '%TGEN8%') AND
                 GR1CODIGO<>17 AND PROSITUACAO='A'
...
View(query_vlx_G175_0623_retroativo)

## CAMPANHA VARILUX ECONOMICA G175 =====================================

CP_G175_VLX_ECONO_0623_RETROATIVO <- 
  query_vlx_G175_0623_retroativo %>% 
  filter(GCLCODIGO==175) %>% 
  mutate(CPF=sub("\\D+", '',PEDAUTORIZOU)) %>% 
  mutate(CPF=sub("\\.", '',CPF)) %>% 
  mutate(CPF=sub("\\-", '',CPF)) %>% 
  mutate(CPF=sub("\\,", '',CPF)) %>% filter(str_detect(PDPDESCRICAO,"AIR"))

View(CP_G175_VLX_ECONO_0623_RETROATIVO)

CP_G175_VLX_ECONO_0623 %>% summarize(v=sum(VRVENDA))

CP_G175_VLX_ECONO_0623 %>% summarize(v=sum(BONUS))


OBS_G175_VLX_ECONO_0623_RETROATIVO <-  paste0("CHILLI BEANS G175 VARILUX ECONOMICA RETROATIVO ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 

## JOIN CPF

LIST_G175_VLX_ECONO_0623_RETROATIVO <- inner_join(CP_G175_VLX_ECONO_0623_RETROATIVO,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF=CPF.y) %>% .[,-11] %>% mutate(OBS=OBS_G175_VLX_ECONO_0623_RETROATIVO)


## pagamentos

PAG_G175_VLX_ECONO_0623_RETROATIVO <- LIST_G175_VLX_ECONO_0623_RETROATIVO %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_G175_VLX_ECONO_0623_RETROATIVO)


View(PAG_G175_VLX_ECONO_0623_RETROATIVO)

View(LIST_G175_VLX_ECONO_0623_RETROATIVO)

## WRITE ON GOOGLE

range_write(data =PAG_G175_VLX_ECONO_0623_RETROATIVO ,ss="1NPLCObC2E0QPsPT8Z7d0omZc_xLwK7ltcieYzy_LkOk",range = "A1",sheet = "RESUMO")

range_write(data =LIST_G175_VLX_ECONO_0623_RETROATIVO ,ss="1NPLCObC2E0QPsPT8Z7d0omZc_xLwK7ltcieYzy_LkOk",range = "A1",sheet = "PEDIDOS")
