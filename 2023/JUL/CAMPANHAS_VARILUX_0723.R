...
CP_VARILUX_G121_0723 <- dbGetQuery(con2,"
...
View(CP_VARILUX_G121_0723)

CP_VARILUX_G121_0723 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_G121_0723 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_G121_0723_G <- inner_join(CP_VARILUX_G121_0723 %>% 
...
View(LIST_VARILUX_G121_0723_G)


range_write(LIST_VARILUX_G121_0723_G,ss="19SdfYG-JiApWajfM7e5whV7VMvZpvji19A_DMm71Gls",range = "A1",
...
LIST_VARILUX_G121_0723 <- inner_join(CP_VARILUX_G121_0723,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>%
  .[,c(-2,-13)] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_G121)

View(LIST_VARILUX_G121_0723)


### pagamentos

PAG_VARILUX_G121_0723 <- LIST_VARILUX_G121_0723 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>%
  mutate(OBS=OBS_VARILUX_G121) 

View(PAG_VARILUX_G121_0723)

# Calc Bonus

PAG_VARILUX_G121_0723 %>% summarize(v=sum(BONUS))
...
CP_206_0723 %>% summarize(v=sum(VRVENDA))

CP_206_0723 %>% summarize(v=sum(BONUS))


OBS_206 <-  paste0("OTICA RH ",CP_206_0723 %>% distinct(CLICODIGO)," ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 

## JOIN CPF

LIST_VARILUX_206_0723 <- inner_join(CP_206_0723,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_206)
...
View(LIST_VARILUX_206_0723)


### pagamentos

PAG_VARILUX_206_0723 <- LIST_VARILUX_206_0723 %>% group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_206)


View(PAG_VARILUX_206_0723)
...
CP_VARILUX_1225_0723 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_1225_0723)

CP_VARILUX_1225_0723 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_1225_0723 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_1225_0723  <- inner_join(CP_VARILUX_1225_0723,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_1225)

View(LIST_VARILUX_1225_0723)


### PAY

PAG_VARILUX_1225_0723 <- LIST_VARILUX_1225_0723 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_1225) 

View(PAG_VARILUX_1225_0723)


PAG_VARILUX_1225_0723 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_360_0723 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_360_0723)

CP_VARILUX_360_0723 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_360_0723 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_360_0723  <- inner_join(CP_VARILUX_360_0723,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_360)

View(LIST_VARILUX_360_0723)


### PAY

PAG_VARILUX_360_0723 <- LIST_VARILUX_360_0723 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)) %>% 
  mutate(OBS=OBS_VARILUX_360) 

View(PAG_VARILUX_360_0723)


PAG_VARILUX_360_0723 %>% summarize(v=sum(BONUS))
...
CP_VARILUX_4576_0723 <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','SR','R')),
...
View(CP_VARILUX_4576_0723)

CP_VARILUX_4576_0723 %>% summarize(v=sum(VRVENDA))

CP_VARILUX_4576_0723 %>% summarize(v=sum(BONUS))
...
LIST_VARILUX_4576_0723  <- inner_join(CP_VARILUX_4576_0723,BASE_CPF,by="CLICODIGO") %>% 
  rename(CPF1=CPF.x) %>% rename(CPF2=CPF.y) %>% 
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_4576)

View(LIST_VARILUX_4576_0723)


### PAY

PAG_VARILUX_4576_0723 <- LIST_VARILUX_4576_0723 %>% 
  group_by(CPF) %>% 
  summarize(BONUS=round(sum(BONUS),2)-75) %>% 
  mutate(OBS=OBS_VARILUX_4576) 

View(PAG_VARILUX_4576_0723)


PAG_VARILUX_4576_0723 %>% summarize(v=sum(BONUS))
...
PAG_VARILUX_0723 <-  
  rbind(
    PAG_VARILUX_G121_0723, 
    PAG_VARILUX_206_0723, 
    PAG_VARILUX_1225_0723,
    PAG_VARILUX_360_0723,
    PAG_VARILUX_4576_0723
  ) %>% mutate(PGTO_MINIMO=if_else(BONUS>=100,"S","N")) %>% 
  mutate(TIPO="VARILUX")


View(PAG_VARILUX_0723)

PAG_VARILUX_0723 %>% summarize(v=sum(BONUS))

range_write(PAG_VARILUX_0723,ss="1bgKkW2aReox24kSi0FvsUQD-b0rOk9RkCXrb66hFhpM",range = "A15",
...
LIST_VARILUX_0723 <-  rbind(
  LIST_VARILUX_G121_0723,
  LIST_VARILUX_206_0723,
  LIST_VARILUX_1225_0723, 
  LIST_VARILUX_360_0723,
  LIST_VARILUX_4576_0723
) 



View(LIST_VARILUX_0723)

LIST_VARILUX_0723 %>% summarize(v=sum(BONUS))

range_write(LIST_VARILUX_0723,ss="1bgKkW2aReox24kSi0FvsUQD-b0rOk9RkCXrb66hFhpM",
...
CREDITO_CARTOES_VARILUX_0723 <- left_join(PAG_VARILUX_0723 %>%
...
View(CREDITO_CARTOES_VARILUX_0723)

## EXCLUI SEM CARTAO

CREDITO_CARTOES_VARILUX_0723_2 <- CREDITO_CARTOES_VARILUX_0723 %>% 
  filter(!is.na(NSERIE)) %>% filter(PGTO_MINIMO=='S')

View(CREDITO_CARTOES_VARILUX_0723_2)


## CRIA BASE DE PAGAMENTO

CREDITO_CARTOES_VARILUX_0723_3 <- CREDITO_CARTOES_VARILUX_0723_2 %>% 
  .[,c(6,1,2,3)] %>% 
  rename_at(1:4, ~ c("Número de Série","CPF","Valor da Carga","Observacao"))


View(CREDITO_CARTOES_VARILUX_0723_3)  

CREDITO_CARTOES_VARILUX_0723_3 %>% summarize(v=sum(`Valor da Carga`))

CREDITO_CARTOES_VARILUX_0723_3 %>% .[duplicated(.$CPF),]


write.csv2(CREDITO_CARTOES_VARILUX_0723_3,
           file = "C:\\Users\\Repro\\Documents\\R\\ADM\\CAMPANHAS_REPRO\\2023\\JUN\\CREDITO_CARTOES_VARILUX_0723.csv",
           row.names=FALSE,quote = FALSE)


left_join(CREDITO_CARTOES_0723_2,ALELO_0723 %>% rename(NSERIE=`Número de Série`),by="NSERIE") %>%.[,c(-4,-5,-6,-7,-8,-9)] %>% View()


## NAO BONIFICADOS ============================================================

NAO_BONIFICADOS_VARILUX_0723 <-
  
  left_join(CREDITO_CARTOES_VARILUX_0723 %>% 
              filter(!is.na(NSERIE)) %>% 
...
range_write("1bgKkW2aReox24kSi0FvsUQD-b0rOk9RkCXrb66hFhpM",data =NAO_BONIFICADOS_VARILUX_0723,
...
VARILUX_EMISSAO_CARTOES_0723 <- CREDITO_CARTOES_VARILUX_0723 %>% 
  filter(is.na(NSERIE)) %>% 
  left_join(.,PARTICIPANTES_CAMPANHA %>% 
              mutate(CPF=as.character(CPF)),by="CPF") %>%  .[,c(1:3,11:14)] 

View(VARILUX_EMISSAO_CARTOES_0723)


range_write("1bgKkW2aReox24kSi0FvsUQD-b0rOk9RkCXrb66hFhpM",data =EMISSAO_CARTOES_0723,
