...
OBS_VARILUX_G121_1023 <- paste0("RINALDI G121 ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 
...
OBS_206_1023 <-  paste0("OTICA RH ",CP_206_1023 %>% distinct(CLICODIGO)," ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 
...
OBS_VARILUX_360_1023 <- paste0("UNIVERSAL 360 ","VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 
...
LIST_VARILUX_360_1023  <- inner_join(CP_VARILUX_360_1023,BASE_CPF,by="CLICODIGO") %>% 
  ...
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_360_1023)
...
OBS_VARILUX_4576_1023 <- paste0("OJO 4576 " ,"VARILUX ",format(floor_date(Sys.Date()-months(1),"month"),"%m%y")) 
...
LIST_VARILUX_4576_1023  <- inner_join(CP_VARILUX_4576_1023,BASE_CPF,by="CLICODIGO") %>% 
  ...
  .[,-12] %>% 
  rename(.,"CPF"="CPF2") %>% 
  mutate(OBS=OBS_VARILUX_4576_1023)
...
write.csv2(CREDITO_CARTOES_VARILUX_1023_3,
           file = "C:\\Users\\REPRO SANDRO\\OneDrive - Luxottica Group S.p.A (1)\\CAMPANHAS ALELO\\2023\\OUT\\CREDITO_CARTOES_VARILUX_1023.csv",
